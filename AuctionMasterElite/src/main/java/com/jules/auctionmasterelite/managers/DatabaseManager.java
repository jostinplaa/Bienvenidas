...
public class DatabaseManager {
    ...
    public record MoneyClaim(int claimId, double amount, String reason) {}
    ...
    private void initializeTables() throws SQLException {
        ...
        String bidsTableSql;
        String itemClaimsTableSql;
        String moneyClaimsTableSql;
        String invitesTableSql;
        String playerPrefsTableSql;
        ...
        if (databaseType == DatabaseType.SQLITE) {
            ...
            itemClaimsTableSql = "CREATE TABLE IF NOT EXISTS item_claims (claim_id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT NOT NULL, item_data BLOB NOT NULL, reason TEXT NOT NULL, timestamp INTEGER NOT NULL);";
            moneyClaimsTableSql = "CREATE TABLE IF NOT EXISTS money_claims (claim_id INTEGER PRIMARY KEY AUTOINCREMENT, player_id TEXT NOT NULL, amount REAL NOT NULL, reason TEXT NOT NULL, timestamp INTEGER NOT NULL);";
            ...
            playerPrefsTableSql = "CREATE TABLE IF NOT EXISTS player_preferences (player_id TEXT PRIMARY KEY, outbid_notification BOOLEAN NOT NULL DEFAULT TRUE, bid_notification BOOLEAN NOT NULL DEFAULT TRUE);";
        } else { // MYSQL
            ...
            itemClaimsTableSql = "CREATE TABLE IF NOT EXISTS item_claims (claim_id INT AUTO_INCREMENT PRIMARY KEY, player_id VARCHAR(36) NOT NULL, item_data LONGBLOB NOT NULL, reason VARCHAR(255) NOT NULL, timestamp BIGINT NOT NULL);";
            moneyClaimsTableSql = "CREATE TABLE IF NOT EXISTS money_claims (claim_id INT AUTO_INCREMENT PRIMARY KEY, player_id VARCHAR(36) NOT NULL, amount DOUBLE NOT NULL, reason VARCHAR(255) NOT NULL, timestamp BIGINT NOT NULL);";
            ...
            playerPrefsTableSql = "CREATE TABLE IF NOT EXISTS player_preferences (player_id VARCHAR(36) PRIMARY KEY, outbid_notification BOOLEAN NOT NULL DEFAULT TRUE, bid_notification BOOLEAN NOT NULL DEFAULT TRUE);";
        }
        ...
        try (Statement stmt = connection.createStatement()) {
            ...
            stmt.execute(itemClaimsTableSql);
            stmt.execute(moneyClaimsTableSql);
            stmt.execute(playerPrefsTableSql);
    ...
    public void saveMoneyClaim(UUID playerId, double amount, String reason) {
        if (connection == null) return;
        String sql = "INSERT INTO money_claims(player_id, amount, reason, timestamp) VALUES(?,?,?,?)";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, playerId.toString());
            ...
            plugin.getLogger().log(Level.SEVERE, "Could not save money claim for player " + playerId, e);
        }
    }

    public List<MoneyClaim> getPlayerMoneyClaims(UUID playerId) {
        if (connection == null) return new ArrayList<>();
        List<MoneyClaim> claims = new ArrayList<>();
        String sql = "SELECT claim_id, amount, reason FROM money_claims WHERE player_id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, playerId.toString());
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    claims.add(new MoneyClaim(rs.getInt("claim_id"), rs.getDouble("amount"), rs.getString("reason")));
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Could not load money claims for player " + playerId, e);
        }
        return claims;
    }

    public void deleteMoneyClaim(int claimId) {
        if (connection == null) return;
        String sql = "DELETE FROM money_claims WHERE claim_id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setInt(1, claimId);
            pstmt.executeUpdate();
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Could not delete money claim " + claimId, e);
        }
    }

    public PlayerPreferences loadPlayerPreferences(UUID playerId) {
        if (connection == null) return new PlayerPreferences(playerId); // Return default if DB is offline
        String sql = "SELECT * FROM player_preferences WHERE player_id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, playerId.toString());
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    return new PlayerPreferences(
                            playerId,
                            rs.getBoolean("outbid_notification"),
                            rs.getBoolean("bid_notification")
                    );
                }
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Could not load preferences for player " + playerId, e);
        }
        // If no record found, return a new object with default values
        return new PlayerPreferences(playerId);
    }

    public void savePlayerPreferences(PlayerPreferences prefs) {
        if (connection == null) return;
        // Use UPSERT logic (INSERT OR REPLACE for SQLite, INSERT ... ON DUPLICATE KEY UPDATE for MySQL)
        String sql = (databaseType == DatabaseType.SQLITE)
                ? "INSERT OR REPLACE INTO player_preferences (player_id, outbid_notification, bid_notification) VALUES (?, ?, ?)"
                : "INSERT INTO player_preferences (player_id, outbid_notification, bid_notification) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE outbid_notification = VALUES(outbid_notification), bid_notification = VALUES(bid_notification)";

        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, prefs.getPlayerId().toString());
            pstmt.setBoolean(2, prefs.hasOutbidNotification());
            pstmt.setBoolean(3, prefs.hasBidNotification());
            pstmt.executeUpdate();
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Could not save preferences for player " + prefs.getPlayerId(), e);
        }
    }
}
