...
public class CreateAuctionMenu extends GUI {
    ...
    private void handleAuctionCreation(Player player) {
        ...
        // Cooldown Check
        if (!player.hasPermission("auctionmaster.limits.bypass")) {
            long remainingCooldown = plugin.getAuctionManager().getRemainingCooldown(player.getUniqueId());
            if (remainingCooldown > 0) {
                MessageUtil.sendMessage(player, "creation-cooldown", "time", TimeUtil.formatDuration(remainingCooldown));
                return;
            }
        }

        // Max Auctions Check
        int maxAuctions = plugin.getConfig().getInt("auction-limits.max-per-player", 10);
        if (maxAuctions > 0 && !player.hasPermission("auctionmaster.limits.bypass")) {
            if (plugin.getAuctionManager().getAuctionsBySeller(player.getUniqueId()).size() >= maxAuctions) {
                MessageUtil.sendMessage(player, "creation-limit-reached");
                return;
            }
        }

        // Blacklist Check
        List<String> blacklist = plugin.getConfig().getStringList("item-restrictions.blacklist");
        if (blacklist.contains(itemToAuction.getType().toString())) {
            MessageUtil.sendMessage(player, "creation-item-blacklisted");
            return;
        }

        // Price Controls Check
        double minPrice = plugin.getConfig().getDouble("price-controls.minimum-starting-bid", 1.0);
        double maxPrice = plugin.getConfig().getDouble("price-controls.maximum-starting-bid", -1.0);
        if (price < minPrice) {
            MessageUtil.sendMessage(player, "creation-price-too-low", "price", String.format("%.2f", minPrice));
            return;
        }
        if (maxPrice > 0 && price > maxPrice) {
            MessageUtil.sendMessage(player, "creation-price-too-high", "price", String.format("%.2f", maxPrice));
            return;
        }
